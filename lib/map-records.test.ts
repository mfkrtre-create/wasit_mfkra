...
describe("map records", () => {
  it("converts valid coordinate records to GeoJSON and excludes invalid coordinates", () => {
    ...
    expect(geojson.features[0]).toMatchObject({
      ...
      properties: { id: "offer-1", recordType: "offer", status: "active", propertyType: "فيلا" },
    });
    expect(geojson.features.some((feature) => feature.properties.id === "missing-coordinates")).toBe(false);
  });

  it("uses approved marker colors for active offers, active requests, and closed records", () => {
  ...
  it("filters visible features by query, status, and city", () => {
    expect(filterMapRecords(records, { query: "النرجس", status: "active", city: "الرياض" })).toHaveLength(1);
    expect(filterMapRecords(records, { query: "", status: "closed", city: "الرياض" })).toHaveLength(1);
    expect(filterMapRecords(records, { query: "", status: "active", city: "جدة" })).toHaveLength(0);
  });
